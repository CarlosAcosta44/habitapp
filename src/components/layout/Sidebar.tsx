...
export function Sidebar({ user }: SidebarProps) {
  ...
  return (
    <aside className="w-64 h-full bg-slate-900 dark:bg-slate-950 flex flex-col text-slate-300 transition-colors border-r border-slate-800">
          ...
          <div className="relative">
            <div className="w-12 h-12 rounded-full overflow-hidden bg-slate-700">
              {user?.avatar_url ? (
                ...
                <div className="w-full h-full flex items-center justify-center text-xl font-bold text-white bg-indigo-600">
                  {user?.full_name?.[0] || 'U'}
                </div>
              )}
            </div>
            <div className="absolute bottom-0 right-0 w-3.5 h-3.5 bg-indigo-300 rounded-full border-2 border-slate-900" />
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-bold text-indigo-200 truncate uppercase tracking-wider leading-tight pb-1">
      ...
      <div className="px-6 mb-6">
        <div className="relative">
          ...
          <input 
            type="text" 
            placeholder="Buscar..." 
            className="w-full bg-slate-800 dark:bg-slate-900 text-sm text-white rounded-lg pl-10 pr-4 py-2 border border-slate-700 focus:outline-none focus:border-indigo-500 transition-colors placeholder:text-slate-500"
          />
        </div>
      </div>
      ...
      {/* Bottom Footer Area */}
      <div className="px-4 pb-6 mt-auto flex flex-col space-y-1 border-t border-slate-800 pt-4">
        ...
        <div className="flex items-center justify-between px-3 py-2.5 mt-2">
          ...
          <button 
            ...
            className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors focus:outline-none ${isDarkMode ? 'bg-indigo-500' : 'bg-slate-600'}`}
          >
